...
# plots
  ...
  set multiplot layout 2, 2 title sprintf("Partial Waves for {/Symbol l} = 0, ..., %i", LMAX)

  set title "{{/Symbol Y}_{/Symbol l}}(r)"
  set key bottom right
  set yrange [*:*]
  plot \
    for [i = 2:LMAX] wavefunctions u 1:i ls i lw 2 t sprintf("%i", i-1)

  set title "{|{/Symbol Y}_{/Symbol l}|}^{2}(r)"
  set key bottom right
  set yrange [*:*]
  plot \
    for [i = 2:LMAX] wavefunctions u 1:(column(i) ** 2) ls i lw 2 t sprintf("%i", i-1)

  set title "{J_{/Symbol l}}(r)"
  set key top right
  set yrange [0:3]
  plot \
    for [i = 2:LMAX] coulomb u 1:i ls i lw 2 t sprintf("%i", i-1), \
  ...
  set yrange [-3:0]
  plot \
    for [i = 2:LMAX] potential u 1:i ls i lw 2 t sprintf("%i", i-1), \
    coulomb u 1:(-2*J(column(1))) lc black lw 2 t sprintf("-2/r")
