subroutine construct_1el_basis_nr(Number_one_electron_func, hf_file, curve_file)
  use input_data
  use sturmian_class
  use vnc_module
  ...
  character(len = *) , intent(in) :: curve_file
  logical :: prediag
  type(core_state) :: core

  call core%read_from(hf_file)
...
!> prediag: true -> kinetic energy already diagonalised
      ...
      !< core potential (coulomb + exchange)
      do ni = 1, nd
        i = no(ni)
        pi => bst%b(i)
        ...
        do nj = 1, nd
          ...
          H(ni,nj) = H(ni,nj) + &
              core%coulomb_me(pi, pj) + core%exchange_me(pi, pj)
